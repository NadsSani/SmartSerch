package com.starwings.app.smartserchapp.CheckVer;
...
import com.starwings.app.smartserchapp.BuildConfig;
import android.util.Log;

import com.loopj.android.http.AsyncHttpClient;
import com.loopj.android.http.FileAsyncHttpResponseHandler;
import com.loopj.android.http.HttpGet;
import com.starwings.app.smartserchapp.HomePageActivity;
...
public class WVersionManager implements IWVersionManager {
	...
	public WVersionManager(HomePageActivity act) {
		this.activity = act;
	...
	public void checkVersion() {
		...
		if (currentTimeStamp > reminderTimeStamp) {
			...
			client.get(getVersionContentUrl(), new FileAsyncHttpResponseHandler(/* Context */ activity) {

				@Override
				public void onFailure(int statusCode, Header[] headers, Throwable throwable, File file) {
					Log.e("Temp",throwable.getClass().getName().toString());
				}
				@Override
	...
	private void showResponse(File responseBody) throws Exception {
	...
	private void showDialog() {
		...
		builder.setMessage("An updated version is available in the playstore. Check it");
		switch (mMode) {
			...
			if (statusCode != HttpStatus.SC_OK) {
				...
				try {
					...
					if (mOnReceiveListener == null || mOnReceiveListener.onReceive(statusCode, result)) {
						...
						Log.e("NewVersion",""+mVersionCode);
						Log.e("CurrentVersion",""+getCurrentVersionCode());
	...
	private void updateSharedPreference(boolean b) {
		SharedPreferences appversion=activity.getSharedPreferences("appPreference",Context.MODE_PRIVATE);
		SharedPreferences.Editor editor=appversion.edit();
		editor.putBoolean("updatestatus",b);
		editor.commit();
	}
